...
impl Token {
    ...
    pub fn tokens(&mut self, syntax: &Syntax, text: &str) -> Vec<Self> {
        let mut tokens: Vec<Self> = text.chars().flat_map(|c| self.automata(c, syntax)).collect();
        ...
        match (self.ty, Ty::from(c)) {
            ...
            (Ty::Literal, _) => match c {
                ...
                c if !c.is_alphanumeric() && !SEPARATORS.contains(&c) => {
                    ...
                    self.buffer.push(c);
                    self.ty = if QUOTES.contains(&c) { Ty::Str(c) } else { Ty::Punctuation(c) };
                }
                _ => {
                    self.buffer.push(c);
                    self.ty = {
            ...
            (Ty::Punctuation(_), _) => {
                if !(syntax.comment.starts_with(&self.buffer) || syntax.comment_multiline[0].starts_with(&self.buffer)) {
                    tokens.extend(self.drain(self.ty));
                    tokens.extend(self.first(c, syntax));
                } else {
                    self.buffer.push(c);
