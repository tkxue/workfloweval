...
use crate::egui_ltree_lib::{Action, DirPosition, IndentHintStyle, NodeBuilder, RowLayout, TreeView, TreeViewBuilder, TreeViewState};
use data::*;
...
fn main() {
    ...
    wasm_bindgen_futures::spawn_local(async {
        ...
        let start_result = eframe::WebRunner::new()
            .start(canvas, web_options, Box::new(|cc| Ok(Box::new(new_app(cc)))))
            .await;
...
impl eframe::App for MyApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::TopBottomPanel::bottom(Id::new("footer")).show(ctx, |ui| ui.label(format!("App version: {}", env!("CARGO_PKG_VERSION"))));
        egui::SidePanel::left(Id::new("tree view")).resizable(true).show(ctx, |ui| {
            ScrollArea::both()
                .scroll([self.settings.scroll_horizontal, self.settings.scroll_vertical])
        ...
        let opened_nodes = self.show_windows_for.iter().map(|node_id| *node_id).collect::<Vec<_>>();
        for node_id in opened_nodes {
            let mut open = true;
            self.tree.find_mut(&node_id, &mut |node| {
                egui::Window::new(node.name()).id(Id::new(node_id)).open(&mut open).show(ctx, |ui| {
                    show_node_content(ui, node);
                });
            });
...
fn show_tree_view(ui: &mut Ui, app: &mut MyApp) -> Response {
    ...
    let (response, actions) = TreeView::new(ui.make_persistent_id("Names tree view"))
        ...
        .min_width(if app.settings.min_width_enabled { app.settings.min_width } else { 0.0 })
        .min_height(if app.settings.min_height_enabled { app.settings.min_height } else { 0.0 })
    ...
    if app.settings.show_size {
        ui.painter().rect_stroke(response.rect, 0.0, (1.0, Color32::BLACK), egui::StrokeKind::Inside);
    }
...
fn show_node(builder: &mut TreeViewBuilder<Uuid>, node: &Node, actions: &mut Vec<ContextMenuActions>) {
    match node {
        Node::Directory(dir) => show_dir(builder, dir, actions),
        Node::File(file) => show_file(builder, file, actions),
    }
}
fn show_dir(builder: &mut TreeViewBuilder<Uuid>, dir: &Directory, actions: &mut Vec<ContextMenuActions>) {
    let mut node = NodeBuilder::dir(dir.id).label(&dir.name).activatable(dir.activatable).context_menu(|ui| {
        ui.set_width(100.0);
    ...
    if dir.custom_closer {
        node = node.closer(|ui, state| {
            ...
            if state.is_open {
                ...
                egui::Image::new(egui::include_image!("folder.png")).tint(color).paint_at(ui, ui.max_rect());
            }
        });
    }
    builder.node(node);
...
fn show_file(builder: &mut TreeViewBuilder<Uuid>, file: &File, actions: &mut Vec<ContextMenuActions>) {
    let parent_node = builder.parent_id().expect("All nodes should have a parent").clone();
    let mut node = NodeBuilder::leaf(file.id).label(&file.name).activatable(file.activatable).context_menu(|ui| {
        ui.set_width(100.0);
    ...
    egui::Grid::new("settings grid").show(ui, |ui| {
        ...
        ui.horizontal(|ui| {
            ...
            ui.add_enabled_ui(override_enabled, |ui| {
                let mut override_striped_value = settings.override_striped.unwrap_or(ui.visuals().striped);
        ...
        ui.horizontal(|ui| {
            ...
            ui.add_enabled_ui(override_enabled, |ui| {
                ...
                let res = ui.add(egui::DragValue::new(&mut override_indent_value).range(0.0..=f32::INFINITY));
                if res.changed() && override_enabled {
        ...
        egui::ComboBox::from_id_salt("row layout combo box")
            ...
            .show_ui(ui, |ui| {
                ...
                ui.selectable_value(&mut settings.row_layout, RowLayout::CompactAlignedLabels, "Compact aligned labels");
                ui.selectable_value(&mut settings.row_layout, RowLayout::AlignedIcons, "Aligned icons");
                ui.selectable_value(&mut settings.row_layout, RowLayout::AlignedIconsAndLabels, "Aligned icons and labels");
            });
        ui.end_row();
