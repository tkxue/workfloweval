...
use crate::egui_ltree_lib::{Action, Activate, TreeView, TreeViewState};
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::SidePanel::left(Id::new("left")).show(ctx, |ui| {
            egui::ScrollArea::both().show(ui, |ui| {
                let (_response, actions) = TreeView::new(ui.make_persistent_id("Names tree view")).show_state(ui, &mut self.tree, |builder| {
                    builder.dir(0, "Root");
                    builder.dir(1, "Foo");
                    builder.leaf(2, "Ava");
                    builder.dir(3, "Bar");
                ...
                for action in actions {
                    match action {
                        Action::Activate(Activate { selected, modifiers: _ }) => {
