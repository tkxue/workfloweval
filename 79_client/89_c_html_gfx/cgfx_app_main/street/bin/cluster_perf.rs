...
use walkers::sources;
use walkers::{
    HttpOptions, HttpTiles, Map, MapMemory, Position, Projector, lon_lat,
};
use walkers_extras::{
...
impl ClusterApp {
    ...
    fn map_center() -> Position {
        walkers::lon_lat(-73.9822, 40.7685)
    }
    ...
    fn rebuild_plugin(&mut self) {
        let plugin =
            GroupedPlacesTree::new(self.points.clone(), DemoClusterGroup)
...
impl walkers::Plugin for StatsHandle {
    fn run(
        ...
        let (clusters, max_size) =
            self.inner.draw_with_stats(ui, response, projector, memory);
