...
pub fn poland_borders() -> KmlLayer {
    let data = include_bytes!("/r/data/vids/CommonPlace_20260114.kml.gz");
    let t2 = e_api::Tgz_Util::gunzip(data).unwrap();
    let t3 = str::from_utf8(t2.as_ref()).unwrap();
    let features = parse_kml(t3).unwrap();
...
pub fn outgym_umea_layer() -> KmlLayer {
    let features =
        parse_kml(include_str!("../assets/utegym-umea.kml")).unwrap();
