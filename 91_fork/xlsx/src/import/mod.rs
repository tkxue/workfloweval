...
fn load_from_excel(
    ...
    let name = file_path
        .file_stem()
        .ok_or_else(|| {
            XlsxError::IO("Could not extract workbook name".to_string())
        })?
        .to_string_lossy()
        .to_string();
...
pub fn load_from_icalc(file_name: &str) -> Result<Model, XlsxError> {
    let contents = fs::read(file_name).map_err(|e| {
        XlsxError::IO(format!("Could not extract workbook name: {e}"))
    })?;
    let workbook: Workbook = bitcode::decode(&contents)
        .map_err(|e| XlsxError::IO(format!("Failed to decode file: {e}")))?;
    Model::from_workbook(workbook, "en").map_err(XlsxError::Workbook)
}
