...
fn from_a1_to_rc(
    ...
    let locale = ironcalc_base::locale::get_default_locale(); // Locale::new("en");
    let language = ironcalc_base::language::get_default_language();

    let mut parser = Parser::new(worksheets.to_owned(), defined_names, tables, &locale, &language);
    let cell_reference =
