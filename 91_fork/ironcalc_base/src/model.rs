...
use crate::{
    ...
    expressions::{
        ...
        parser::{
            ...
            stringify::{
                rename_defined_name_in_node, to_localized_string, to_rc_format,
            },
            Node, Parser,
        },
        token::{
            get_error_by_name, Error, OpCompare, OpProduct, OpSum, OpUnary,
        },
        types::*,
        utils::{
            self, is_valid_column_number, is_valid_identifier, is_valid_row,
        },
    },
    formatter::{
...
pub struct Model<'a> {
    ...
    pub(crate) parsed_defined_names:
        HashMap<(Option<u32>, String), ParsedDefinedName>,
...
impl<'a> Model<'a> {
        ...
        match node {
            ...
            Node::ImplicitIntersection {
                ...
                _ => CalcResult::new_error(
                    ...
                    cell,
                    format!(
                        "Error with Implicit Intersection in cell {cell:?}"
                    ),
                ),
            },
    ...
    pub(crate) fn evaluate_node_in_context(
        ...
        match node {
            ...
            StringKind(value) => {
                CalcResult::String(value.replace(r#""""#, r#"""#))
            }
            ...
            WrongReferenceKind { .. } => CalcResult::new_error(
                Error::REF,
                cell,
            ...
            InvalidFunctionKind { name, args: _ } => CalcResult::new_error(
                Error::NAME,
                cell,
            ...
            ErrorKind(kind) => {
                CalcResult::new_error(kind.clone(), cell, "".to_string())
            }
            ParseErrorKind {
                formula,
                message,
    ...
    fn set_cell_value(
        ...
        let cell =
            &self.workbook.worksheets[sheet as usize].sheet_data[&row][&column];
            ...
            match result {
                ...
                CalcResult::Boolean(value) => {
                    *self.workbook.worksheets[sheet as usize]
                        ...
                        .get_mut(&column)
                        .expect("expected a column") =
                        Cell::CellFormulaBoolean { f, s, v: *value };
                }
                CalcResult::Error {
                    error,
                    origin,
                        ...
                        && left.column == right.column
                    {
                        ...
                        *self.workbook.worksheets[sheet as usize]
                            ...
                            Cell::CellFormulaError {
                                ...
                                s,
                                o,
                                m: "Implicit Intersection not implemented"
                                    .to_string(),
                                ei: Error::NIMPL,
                            };
                    }
    ...
    pub(crate) fn evaluate_cell(
        ...
        let row_data = match self.workbook.worksheets
            [cell_reference.sheet as usize]
            .sheet_data
        ...
        match cell.get_formula() {
            Some(f) => {
                ...
                let node = &self.parsed_formulas[cell_reference.sheet as usize]
                    [f as usize]
                    .clone();
                let result =
                    self.evaluate_node_in_context(node, cell_reference);
    ...
    pub fn from_bytes(
        ...
        let workbook: Workbook = bitcode::decode(s)
    ...
    pub fn from_workbook(
        ...
        let tz: Tz = workbook.settings.tz.parse().map_err(|_| {
            format!("Invalid timezone: {}", workbook.settings.tz)
        })?;
    ...
    pub fn move_cell_value_to_area(
        ...
        if source.column < area.column
            || source.column >= area.column + area.width
        {
            return Err("Source is outside the area".to_string());
        }
            ...
            Some(cell) => match cell.get_formula() {
                ...
                Some(i) => {
                    ...
                    let cell_ref = CellReferenceRC {
                        sheet: self.workbook.worksheets[sheet as usize]
                            .get_name(),
                        row: target_row,
    ...
    pub fn extend_copied_value(
        ...
        let source_sheet_name =
            match self.workbook.worksheets.get(source.sheet as usize) {
                Some(ws) => ws.get_name(),
                None => {
                    return Err("Invalid worksheet index".to_owned());
                }
            };
        let target_sheet_name =
            match self.workbook.worksheets.get(target.sheet as usize) {
                Some(ws) => ws.get_name(),
                None => {
                    return Err("Invalid worksheet index".to_owned());
                }
    ...
    pub fn set_user_input(
        ...
        if let Some(new_value) = value.strip_prefix('\'') {
            ...
            let new_style =
                if common::value_needs_quoting(new_value, self.language) {
                    self.workbook
                        .styles
                        .get_style_with_quote_prefix(style_index)?
                } else {
            ...
            self.set_cell_with_string(
                sheet, row, column, new_value, new_style,
            )?;
        } else {
            ...
            if let Some(formula) = value.strip_prefix('=') {
                let formula_index = self.set_cell_with_formula(
                    sheet,
                    row,
                    column,
                    formula,
                ...
                let parsed_formula = &self.parsed_formulas[sheet as usize]
                    [formula_index as usize];
                if let Some(units) =
                    self.compute_node_units(parsed_formula, &cell)
                {
                    let new_style_index =
                        self.workbook.styles.get_style_with_format(
                            new_style_index,
                            &units.get_num_fmt(),
                        )?;
                    let style =
                        self.workbook.styles.get_style(new_style_index)?;
                ...
                if let Ok((v, number_format)) = parse_formatted_number(
                    &value,
                    &currencies,
                    decimal_separator,
                    group_separator,
                    ...
                    let worksheet = self.workbook.worksheet_mut(sheet)?;
                    worksheet.set_cell_with_number(
                        row,
                        column,
                        v,
                        new_style_index,
                ...
                if let Ok(v) = value.to_lowercase().parse::<bool>() {
                    let worksheet = self.workbook.worksheet_mut(sheet)?;
                    worksheet.set_cell_with_boolean(
                        row,
                        column,
                        v,
                        new_style_index,
    ...
    fn set_cell_with_formula(
        ...
        if let Node::ParseErrorKind { .. } = parsed_formula {
            let new_parsed_formula =
                self.parser.parse(&format!("{formula})"), &cell_reference);
    ...
    pub fn get_cell_value_by_ref(
        ...
        let cell_reference = match self.parse_reference(cell_ref) {
            Some(c) => c,
            None => {
                return Err(format!("Error parsing reference: '{cell_ref}'"))
            }
        };
    ...
    pub fn get_cell_value_by_index(
        ...
        let cell = self
            ...
            .unwrap_or_default();
        let cell_value =
            cell.value(&self.workbook.shared_strings, self.language);
        Ok(cell_value)
    }
    ...
    pub fn get_formatted_cell_value(
        ...
        match self.workbook.worksheet(sheet_index)?.cell(row, column) {
            Some(cell) => {
                let format =
                    self.get_style_for_cell(sheet_index, row, column)?.num_fmt;
                let formatted_value = cell.formatted_value(
                    &self.workbook.shared_strings,
                    self.language,
                    |value| format_number(value, &format, self.locale).text,
                );
                Ok(formatted_value)
            }
            None => Ok("".to_string()),
    ...
    pub fn get_localized_cell_content(
        ...
        match cell.get_formula() {
            Some(formula_index) => {
                let formula = &self.parsed_formulas[sheet as usize]
                    [formula_index as usize];
                let cell_ref = CellReferenceRC {
                    sheet: worksheet.get_name(),
                    row,
    ...
    pub fn get_sheet_markup(&self, sheet: u32) -> Result<String, String> {
        ...
        for row in 1..(dimension.max_row + 1) {
            ...
            for column in 1..(dimension.max_column + 1) {
                let mut cell_markup =
                    match self.get_cell_formula(sheet, row, column)? {
    ...
    pub fn set_frozen_rows(
        ...
        if let Some(worksheet) =
            self.workbook.worksheets.get_mut(sheet as usize)
        {
            if frozen_rows < 0 {
                ...
                if new_name != df.name {
                    ...
                    for worksheet in worksheets {
                        ...
                        for formula in &worksheet.shared_formulas {
                            let mut t =
                                self.parser.parse(formula, &cell_reference);
                            rename_defined_name_in_node(
                                &mut t, name, scope, new_name,
                            );
            ...
            for col in cols {
                if column >= col.min && column <= col.max {
                    if let Some(style_index) = col.style {
                        let style =
                            self.workbook.styles.get_style(style_index)?;
    ...
    pub fn get_fmt_settings(&self) -> FmtSettings {
        ...
        let short_date_example =
            format_number(day_example, short_date, self.locale).text;
        let long_date_example =
            format_number(day_example, long_date, self.locale).text;
        // Number format ("#,##0.###")
        ...
        let number_example =
            format_number(1234.567, &number_fmt, self.locale).text;
        FmtSettings {
            currency,
